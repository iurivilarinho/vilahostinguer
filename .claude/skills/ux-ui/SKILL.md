...
```yaml
argument-hint: screen, flow, or usability/design problem
tools:
  - read
  - search
```
...
Before proposing new UI components, read the `component-creation` skill to understand existing patterns (variants, compound patterns, accessibility guidelines).
