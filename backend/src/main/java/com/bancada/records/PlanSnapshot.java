...
public record PlanSnapshot(String name, Double cpuLimit, Integer memoryMb, Integer diskGb, int backupSlots,
                           BigDecimal priceMonthly, boolean active) {

    public PlanSnapshot(Plan plan) {
        this(plan.getName(), plan.getCpuLimit(), plan.getMemoryMb(), plan.getDiskGb(), plan.getBackupSlots(),
