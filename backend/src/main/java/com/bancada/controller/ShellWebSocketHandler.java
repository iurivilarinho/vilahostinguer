...
/**
 * Bridges an xterm.js terminal to a PTY on a device (SSH shell or docker exec).
 *
 * <p>Protocol: the browser sends JSON text frames, {@code {"type":"input","data":"..."}} for keys
 * and {@code {"type":"resize","cols":120,"rows":32}} for size changes. The output goes back as raw
 * binary frames, left for xterm.js to decode, so a multi-byte character split between two reads is
 * never mangled here. Subclasses only decide which PTY a connection opens.
 */
public abstract class ShellWebSocketHandler extends TextWebSocketHandler {
    ...
    protected static final int DEFAULT_ROWS = 32;

    private final ObjectMapper objectMapper;
    private final Map<String, Channel> shells = new ConcurrentHashMap<>();

    protected ShellWebSocketHandler(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    /** Opens (without connecting) the PTY for this connection; throw to refuse it with a message. */
    protected abstract ShellSession open(WebSocketSession session, Map<String, String> query, int columns, int rows) throws JSchException;
    ...
    public void afterConnectionEstablished(WebSocketSession rawSession) throws IOException {
        ...
        try {
            ShellSession shell = open(rawSession, query, parseOrDefault(query.get("cols"), DEFAULT_COLUMNS),
                parseOrDefault(query.get("rows"), DEFAULT_ROWS));
            InputStream output = shell.channel().getInputStream();
            shell.channel().connect(CONNECT_TIMEOUT_MS);
            shells.put(rawSession.getId(), shell.channel());
            Thread reader = new Thread(() -> pump(output, session, shell.channel()), shell.threadName());
            reader.setDaemon(true);
            reader.start();
        } catch (NumberFormatException exception) {
            closeWithMessage(session, "Dispositivo, máquina ou servidor não informado.");
    ...
    private void pump(InputStream output, WebSocketSession session, Channel shell) {
        ...
        try {
            ...
            if (session.isOpen()) {
                try {
                    session.close(CloseStatus.NORMAL.withReason("Sessão encerrada"));
                } catch (IOException exception) {
